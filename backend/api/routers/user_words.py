...
from ...crud.models import user_word_crud
from ... import auth
...
async def create_user_word(
    ...
    try:
        db_user_word = user_word_crud.create(
            obj_in=user_word,
            db=db,
        )
        return db_user_word
...
async def read_user_word(user_word_id: int, db: Session = Depends(get_db)):
    db_user_word = user_word_crud.get(id=user_word_id, db=db)
...
async def read_user_words(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return user_word_crud.get_multi(db=db, skip=skip, limit=limit)
...
async def update_user_word(
    ...
    return user_word_crud.update(db=db, db_obj=db_user_word, obj_in=user_word)
...
async def delete_user_word(user_word_id: int, db: Session = Depends(get_db)):
    db_user_word = user_word_crud.delete(id=user_word_id, db=db)
